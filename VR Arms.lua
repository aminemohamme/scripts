...
EnablePointer = false -- enables the default pointer for clicking things (offset annoyingly unfixable)
ArmStiffness = 15 -- how stiff the arms are
...
Frame.BorderColor3 = Color3.fromRGB(29, 35, 117)
Frame.BorderSizePixel = 50
...
TextLabel.Position = UDim2.new(0.176345274, 0, 0.0797201991, 0)
TextLabel.Size = UDim2.new(0, 400, 0, 150)
TextLabel.Font = Enum.Font.SourceSans
TextLabel.Text = "Made by 1+1=2/rouxhaver"
...
countdown.Position = UDim2.new(0.209429815, 0, 0.56015408, 0)
countdown.Size = UDim2.new(0, 400, 0, 150)
...
for number = 5,1,-1 do
	countdown.Text = "Loading in... "..tostring(number)
	wait(1)
end
ScreenGui:Destroy()
...
for i, v in pairs(workspace[name].Model:GetChildren()) do
...
workspace[name].Model.VarietyShades10.Handle.Mesh:Destroy()
workspace[name].Model.VarietyShades10.Name = "HAT"
workspace[name].Model.VarietyShades02.Handle.Mesh:Destroy()
workspace[name].Model.VarietyShades02.Name = "HAT"

for i, v in pairs(workspace[name].Model:GetChildren()) do
    if v:IsA("Accessory") and v.Name == "HAT" then
...
--thanks infinite yield
local function NoclipLoop()
	if workspace[name] ~= nil then
		for _, child in pairs(workspace[name]:GetDescendants()) do
...
workspace[name].HumanoidRootPart.RootJoint:Destroy()
workspace[name].Torso["Right Shoulder"]:Destroy()
workspace[name].Torso["Left Shoulder"]:Destroy()
workspace[name].Torso["Left Hip"]:Destroy()
workspace[name].Torso["Right Hip"]:Destroy()



Head = workspace[name].Head
Head.Anchored = true
...
Cam.VRCorePanelParts.BottomBar_Part:Destroy()

if Cam.VRCorePanelParts:FindFirstChild('UserGui') then
	Cam.VRCorePanelParts.UserGui.Size *= 0
end

workspace[name].Humanoid.HipHeight = 5



LeftHand = workspace[name].VarietyShades10.Handle
LeftHand.Parent.Name = "USED"
LeftHand.AccessoryWeld:Destroy()


RightHand = workspace[name].VarietyShades02.Handle
RightHand.Parent.Name = "USED"
RightHand.AccessoryWeld:Destroy()


--pointless varibles cause i'm lazy
Rarm = RightHand
Rarm.Anchored = true
...
joint = Instance.new("BallSocketConstraint",workspace)
joint.Attachment0 = workspace[name]["Left Arm"].LeftShoulderAttachment
joint.Attachment1 = workspace[name]["Left Leg"].LeftFootAttachment
joint.MaxFrictionTorque = ArmStiffness

joint = Instance.new("BallSocketConstraint",workspace)
att = Instance.new("Attachment",workspace[name]["Left Leg"])
att.CFrame = CFrame.new(0,1,0)
joint.Attachment0 = workspace[name]["Torso"].LeftCollarAttachment
joint.Attachment1 = att
joint.MaxFrictionTorque = ArmStiffness

joint = Instance.new("BallSocketConstraint",workspace)
att = Instance.new("Attachment",LeftHand)
att.CFrame = CFrame.new(0.6,0,0)
joint.Attachment0 = workspace[name]["Left Arm"].LeftGripAttachment
joint.Attachment1 = att
joint.MaxFrictionTorque = ArmStiffness

joint = Instance.new("BallSocketConstraint",workspace)
att = Instance.new("Attachment",RightHand)
att.CFrame = CFrame.new(0.6,0,0)
joint.Attachment0 = workspace[name]["Right Arm"].RightGripAttachment
joint.Attachment1 = att
joint.MaxFrictionTorque = ArmStiffness

joint = Instance.new("BallSocketConstraint",workspace)
joint.Attachment0 = workspace[name]["Right Arm"].RightShoulderAttachment
joint.Attachment1 = workspace[name]["Right Leg"].RightFootAttachment
joint.MaxFrictionTorque = ArmStiffness

joint = Instance.new("BallSocketConstraint",workspace)
att = Instance.new("Attachment",workspace[name]["Right Leg"])
att.CFrame = CFrame.new(0,1,0)
joint.Attachment0 = workspace[name]["Torso"].RightCollarAttachment
joint.Attachment1 = att
joint.MaxFrictionTorque = ArmStiffness
...
while task.wait() do
    Rarm.CFrame = (Cam.CFrame - Cam.CFrame.p) * (VRService:GetUserCFrame(Enum.UserCFrame.RightHand) - VRService:GetUserCFrame(Enum.UserCFrame.RightHand).p) + Cam.CFrame*(VRService:GetUserCFrame(Enum.UserCFrame.RightHand).p*HeadScale)
    Rarm.CFrame = Rarm.CFrame * CFrame.Angles(math.rad(90),math.rad(0),math.rad(90))

    Larm.CFrame = (Cam.CFrame - Cam.CFrame.p) * (VRService:GetUserCFrame(Enum.UserCFrame.LeftHand) - VRService:GetUserCFrame(Enum.UserCFrame.LeftHand).p) + Cam.CFrame*(VRService:GetUserCFrame(Enum.UserCFrame.LeftHand).p*HeadScale)
    Larm.CFrame = Larm.CFrame * CFrame.Angles(math.rad(90),math.rad(0),math.rad(90))

	Head.CFrame = workspace.CurrentCamera:GetRenderCFrame()
end
